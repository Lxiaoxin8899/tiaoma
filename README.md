...
> 注意：离线模式仅用于演示/开发，认证与数据在本地可被篡改；默认登录口令为 `local`。
...
### Q: Windows 下 `pnpm install` 报 `EBUSY`/`default_app.asar` 被占用怎么办？
A: 先关闭正在运行的 Electron/Node 进程，再删除 `node_modules` 后重装；如果仍被占用，通常是杀毒/索引占用导致，建议重启或将项目目录加入杀毒白名单后再执行安装。
